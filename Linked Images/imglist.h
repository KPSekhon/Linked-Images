...
class ImgList {
  ...
  *********************************************************/
  private:
  ImgNode* createNode(HSLAPixel *currentPixel);
  ImgNode* makeRow(unsigned y, PNG &img);
  void deleteNode(ImgNode* node);
  void deleteRow(ImgNode* start);
  ImgNode* minLumNode(ImgNode* rowstart);
  ImgNode* minHueDiffNode(ImgNode* rowstart);
  PNG RenderGaps(int fillmode,PNG &img) const;
  PNG RenderGaps0(PNG &img) const;
  PNG RenderGaps1(PNG &img) const;
  PNG RenderGaps2(PNG &img) const;
  PNG RenderWithoutGaps(PNG &img) const;
  void setPixel(HSLAPixel *pixel, HSLAPixel setter) const;
  void setSameColourRow(unsigned int x, unsigned int y, HSLAPixel colour, unsigned int skipright, PNG &img)const;
  void setFillTwo(unsigned int x, unsigned int y, HSLAPixel colour, PNG &img) const;
  HSLAPixel averageColour(ImgNode* leftNode, ImgNode* rightNode) const;
  double hueCalculator(double leftHue, double rightHue) const;
  unsigned int currentRowXLength(ImgNode* current) const;
  double angle_between(double a, double b) const;
  ImgNode* makeCopyRow(unsigned int y,const ImgList &otherlist, ImgNode* otherlistStart);  
  ImgNode *copyNode(ImgNode* othernode);
  HSLAPixel differenceCalculator(double diff,ImgNode *leftNode, ImgNode *rightNode) const;
  double hueCalculator2(double leftHue, double rightHue, double diff) const;
